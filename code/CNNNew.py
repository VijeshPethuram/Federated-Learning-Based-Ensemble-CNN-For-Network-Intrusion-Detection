...
class CloudServer:
    ...
    def trainmodel(self):
        ...
        if X_train_preprocessed.shape[1] == 104:  
            X_train_cnn = X_train_preprocessed.reshape(X_train_preprocessed.shape[0], 104, 1)
            X_test_cnn = X_test_preprocessed.reshape(X_test_preprocessed.shape[0], 104, 1)
        else:
            raise ValueError("Expected 104 features after preprocessing, but got: {}".format(X_train_preprocessed.shape[1]))

        for name, model in self.models.items():
            if name == "cnn":
                model.fit(X_train_cnn, y_train, epochs=1, batch_size=32, verbose=1)
    ...
    def predict(self, packet):
        ...
        X_cnn = X_preprocessed.reshape(1, 104, 1)  
        ...
        if ensemble_prediction[0] == "anomaly":
            return "anomaly"
        else:
